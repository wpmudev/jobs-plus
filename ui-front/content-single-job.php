...
<div class="job-single-wrapper">
	...
	<div class="job-meta group">
		<div class="job-date">
			<p class="subheader1"><?php echo esc_html( sprintf( __( 'Posted by: %s on %s', JBP_TEXT_DOMAIN ), get_the_author(), get_the_date() ) ); ?></p>
		</div>

		<ul class="group">
			<li><span class="meta-label"><?php esc_html_e( 'Job Budget', JBP_TEXT_DOMAIN ); ?>
					<br /></span><span id="project-min" class="meta-red "></span><span id="project-dash" class="meta-red "></span><span id="project-max" class="meta-red "></span>
			</li>
			<li><span class="meta-label"><?php esc_html_e( 'This job open for', JBP_TEXT_DOMAIN ); ?>
					<br /></span><span class="meta-green"><?php echo days_hours( get_post_meta( get_the_ID(), JBP_JOB_EXPIRES_KEY, true ) ); ?></span>
			</li>
			<li class="border"><span class="meta-label"><?php esc_html_e( 'Must be complete by', JBP_TEXT_DOMAIN ); ?>
					<br /></span><span class="meta-red"><?php echo do_shortcode( '[ct id="_ct_jbp_job_Due" ]' ); ?></span>
			</li>
			<li>
				<?php
				if ( get_post_meta( get_the_ID(), JBP_JOB_EXPIRES_KEY, true ) > time() ) {
					...
					$terms = get_the_terms( get_the_ID(), 'jbp_category' );
					if ( $terms && ! is_wp_error( $terms ) ) {
						printf( '<a class="job-link" href="%s" >%s</a>', esc_attr( get_term_link( reset( $terms ) ) ), esc_html__( 'More like this...', JBP_TEXT_DOMAIN ) );
					}
				}
				?>
			</li>
	...
	<div id="post-full-<?php the_ID(); ?>" <?php post_class(); ?> >
		...
		<div class="job-item-full">
			...
			<?php the_content(); ?>
			<?php $terms_count = get_the_terms(get_the_ID(),'jbp_skills_tag'); ?>
			<?php if(is_array($terms_count) && count($terms_count)): ?>
			...
			<?php endif; ?>
			<?php
			//check does the job have portfolio
