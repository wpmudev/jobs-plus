...
class JobsExperts_Core_Views_Settings_Expert extends JobsExperts_Framework_Render {
	...
	public function _to_html() {
		...
		<table class="form-table">
			...
			<tr>
				<th>
					<label><?php printf( esc_html__( 'Maximum %s Records per User', JBP_TEXT_DOMAIN ), $pro_labels->singular_name ) ?></label>
				</th>
				<td>
					<?php echo $form->textField( $model, 'expert_max_records' ) ?>
					<br /><span class="description"><?php printf( esc_html__( 'Maximum number of %s profiles for each user.', JBP_TEXT_DOMAIN ), $pro_labels->singular_name ); ?></span>
				</td>
			</tr>

			<tr>
				<th>
					<label><?php printf( esc_html__( '%s Records per Page', JBP_TEXT_DOMAIN ), $pro_labels->singular_name ); ?></label>
				</th>
				<td>
					<?php echo $form->textField( $model, 'expert_per_page' ) ?>

					<br /><span class="description"><?php printf( esc_html__( 'Maximum number of %s profiles per archive page.', JBP_TEXT_DOMAIN ), $pro_labels->singular_name ); ?></span>
				</td>
			</tr>

			<tr>
				<th>
					<label><?php printf( esc_html__( 'Newly Created %s Status Options', JBP_TEXT_DOMAIN ), $pro_labels->singular_name ); ?></label>
				</th>
				<td>
					<label>
						<?php echo $form->radioButton( $model, 'expert_new_expert_status', 'publish' ) ?>

						<?php esc_html_e( 'Published', JBP_TEXT_DOMAIN ); ?>
					</label>
					<br /><span class="description"><?php printf( esc_html__( 'Allow members to publish %s themselves.', JBP_TEXT_DOMAIN ), $pro_labels->name ); ?></span>
					<br />
					<br />
		...
		<table class="form-table">
			<tr>
				...
				<td>
					...
					<br /><span class="description"><?php printf( esc_html__( 'Maximum number of images that can be uploaded to the %s portfolio gallery. Default is 4', JBP_TEXT_DOMAIN ), $pro_labels->name ); ?></span>
				</td>
			</tr>
		...
		<table class="form-table">
			...
			<tr>
				...
				<td>
					<?php $form->hiddenField( $model, 'expert_contact_form_captcha', array( 'value' => 0 ) ) ?>
					<?php $form->checkBox( $model, 'expert_contact_form_captcha', array( 'value' => 1 ) ) ?>
			...
			<tr>
				...
				<td>
					<?php $form->hiddenField( $model, 'expert_cc_sender', array( 'value' => 0 ) ) ?>
					<?php $form->checkBox( $model, 'expert_cc_sender', array( 'value' => 1 ) ) ?>
