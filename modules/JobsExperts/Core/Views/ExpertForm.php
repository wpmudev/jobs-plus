                    ...
                    <div class="social_list">
                        <ul>
                            ...
                            foreach ($plugin->settings()->social_list as $key => $val): ?>
                                ...
                                ?>
                                <li>
                                    <a data-value="<?php echo is_array($found) ? $found['url'] : null ?>"
                                       data-id="<?php echo $key ?>" class="add-social-<?php echo $key ?> popup-edit"
                                       href="#" title="<?php echo $val ?>">
                                        <img style="<?php echo is_array($found) ? 'opacity:1' : null ?>"
                                             src="<?php echo $plugin->_module_url ?>assets/social_icon/<?php echo $key ?>.png">
                                    </a>
                                        ...
                                        jQuery(document).ready(function ($) {
                                            ...
                                            $('.add-social-<?php echo $key ?>').popover({
                                                ...
                                                if (next.hasClass('popover')) {
                                                    ...
                                                    //some field not url
                                                    if ($.inArray(_this.data('id'), ['skype', 'hangouts', 'email']) > -1) {
                                                        form.find('input[type="text"]').removeClass('validate[custom[url]]').val('');
                                                    }
        ...
        <div class="panel panel-default">
            <div class="jbo_pro_social">
                ...
                <div class="panel-body" style="padding: 0">
                        ...
                        <div class="jbp_skill_meta">
                            ...
                            foreach ($skills as $skill):
                                ...
                                <div class="jbp_skillbar_container" style="position: relative">
                                    <i class="dashicons dashicons-trash remove-skill"
                                       style="position: absolute;right:-21px;color: red;cursor: pointer"></i>
                    ...
                    <div class="jbp_pro_contact">
                        <button type="button"
                                class="add-skill btn btn-small btn-primary"><?php _e('Add Skill') ?></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-7 col-xs-12 col-sm-12" style="padding: 0">
            <div class="jbp_pro_main_form">
                ...
                <div class="row full">
                    ...
                    <div class="col-md-12">
                        <?php
                        $desc = new JobsExperts_Framework_EditableForm($model, $edit_icon . '<div class="can-edit">{{biography}}</div>', $edit_icon . '<p class="can-edit">' . __('Tell us about yourself (required, at least 200 characters)', JBP_TEXT_DOMAIN) . '</p>', array(
                            'biography' => array(
                                'type' => 'textArea',
                ...
                <div class="row" style="margin-top: 40px">
                    <div class="col-md-12" style="margin-left: 0">
                        ...
                        <?php if ($plugin->settings()->expert_new_expert_status == 'publish'): ?>
                            <button class="jbp_publish btn btn-small btn-primary" name="status" value="publish"
                                    type="submit"><?php _e('Publish', JBP_TEXT_DOMAIN) ?></button>
                        <?php else: ?>
                            <button class="jbp_publish btn btn-small btn-primary" name="status"
                                    value="pending"><?php _e('Submit for review', JBP_TEXT_DOMAIN) ?></button>
                        <?php endif; ?>
                        <?php if ($plugin->settings()->expert_allow_draft == 1): ?>
                            <button class="jbp_save_draft btn btn-small btn-info" name="status" value="draft"
    ...
    function get_skill_template()
    {
        ...
        <form method="post" class="skill-add-form" style="width: 200px">
            <div class="form-group" style="margin-bottom: 4px">
                <label
                    style="font-size: 12px;font-weight: normal;display: block"><?php _e('Skill', JBP_PLUGIN_URL) ?></label>
                <?php echo JobsExperts_Framework_Form::textField('skill_name', '', array('class' => 'validate[required]', 'style' => 'width:85%')) ?>
            </div>
            <div class="form-group">
                <label
                    style="font-size: 12px;font-weight: normal;display: block"><?php _e('Score', JBP_PLUGIN_URL) ?></label>
                <input name="skill_score"
                       style="padding: 0;margin:0;position: relative;top:3px;width: 88%;display: inline" type="range"
                       min="0" max="100" class="skill_score"/>
