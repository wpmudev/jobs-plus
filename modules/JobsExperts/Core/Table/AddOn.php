...
class JobsExperts_Core_Table_AddOn extends WP_List_Table
{
    ...
    public function column_col_name($item)
    {
        ...
        if (in_array($item['col_id'], $components)) {
            $html .= '<br><a class="plugin" data-type="deactive" data-id="' . esc_attr($item['col_id']) . '" href="#">' . __('Deactivate', JBP_TEXT_DOMAIN) . '</a>';
        } else {
            $html .= '<br><a class="plugin" data-type="active" data-id="' . esc_attr($item['col_id']) . '"  href="#">' . __('Activate', JBP_TEXT_DOMAIN) . '</a>';
        }
    ...
    {
        $data = JobsExperts_AddOn::get_available_components();
        $items = array();
