...
class JobsExpert_Compnents_AdvancedSearch extends JobsExperts_Components {
	...
	function get_job_advance_search_form( $show_search_form = '' ) {
		ob_start();
		?>
		<form class="job_advanced_search_form" method="get" action="<?php echo get_post_type_archive_link( 'jbp_job' ) ?>">
			...
			<table class="table">
				...
				<tr>
					<td></td>
					<td>
						<button type="submit" class="btn btn-xs btn-primary"><?php _e( 'Search', JBP_TEXT_DOMAIN ) ?></button>
						&nbsp;
						<button type="button" class="btn btn-xs cancel_search_form btn-default"><?php _e( 'Cancel', JBP_TEXT_DOMAIN ) ?></button>
					</td>
				</tr>
			</table>
		</form>
			...
			<table class="table">
				...
				<tr>
					...
					<td style="width: 80%">
						...
						<label>
							<input type="radio" value="popular" name="order_by"> <?php _e( 'Most Popular', JBP_TEXT_DOMAIN ) ?>
						</label> &nbsp;&nbsp;
