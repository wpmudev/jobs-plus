...
if ( ! class_exists( 'Jobs_Plus_Admin' ) ):
	...
	class Jobs_Plus_Admin extends Jobs_Plus_Core {
		...
		function __construct() {
			...
			add_action( 'init', array( $this, 'create_virtual_pages' ), 20 );
		...
		function admin_menu_page_job() {
			...
			switch ( $current_tab ) {
				...
				case 'shortcodes':
					...
					break;
				case 'convert':
					include $this->plugin_dir . 'ui-admin/convert.php';
					break;
				case 'page_creation':
		...
		function render_tabs( $current_tab = 'settings' ) {
			...
			<h2 class="nav-tab-wrapper">
				<?php foreach ( $tabs as $tab => $title ):
					...
					?>
					<a class="nav-tab <?php echo $class ?>" href="<?php esc_attr_e( "?post_type=jbp_job&page=jobs-plus-menu&tab=$tab" ); ?>"><img src="<?php echo $this->plugin_url . "img/{$tab}16.png"; ?>" /> <?php echo $title; ?>
					</a>
				<?php endforeach; ?>
			</h2>
			...
			if ( ! empty( $action ) ) {
				...
				switch ( $action ) {
					...
					case 'jbp_create_add_pro_page':
						...
						break;
					case 'jbp_create_full_page':
						check_admin_referer( 'jbp_create_full_page' );
						if ( $this->get_setting( 'pages_define->job_board', false ) === false || get_post($this->get_setting( 'pages_define->job_board')) == null) {

							$post_content = '[jbp-job-post-btn text="Post a Job" view="both"][jbp-job-browse-btn text="Find a Pro" view="both"][jbp-pro-browse-btn text="Browse Jobs" view="both"][jbp-pro-post-btn text="Become a Pro" view="both"]';
							$args         = array(
								'post_title'     => sprintf( 'Job board' ),
								'post_status'    => 'publish',
								'post_author'    => get_current_user_id(),
								'post_type'      => 'page',
								'post_content'   => $post_content,
							...
							$id           = wp_insert_post( $args );
							//update new page to setting
							$settings                              = $settings = get_option( $this->settings_name );
							$settings['pages_define']['job_board'] = $id;
							update_option( $this->settings_name, $settings );
						}
