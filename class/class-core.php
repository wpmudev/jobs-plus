...
if ( ! class_exists( 'Jobs_Plus_Core' ) ):
	class Jobs_Plus_Core {
		...
		function __get( $name ) {
			$result = false;
			switch ( $name ) {
				case 'job_archive_page_id':
					$result = $this->find_page_id( $this->_job_archive_page_id, JBP_JOB_VIRTUAL_KEY, JBP_JOB_ARCHIVE_FLAG );
					break;
				case 'job_taxonomy_page_id':
					$result = $this->find_page_id( $this->_job_taxonomy_page_id, JBP_JOB_VIRTUAL_KEY, JBP_JOB_TAXONOMY_FLAG );
					break;
				case 'job_contact_page_id':
					$result = $this->find_page_id( $this->_job_contact_page_id, JBP_JOB_VIRTUAL_KEY, JBP_JOB_CONTACT_FLAG );
					break;
				case 'job_search_page_id':
					$result = $this->find_page_id( $this->_job_search_page_id, JBP_JOB_VIRTUAL_KEY, JBP_JOB_SEARCH_FLAG );
					break;
				case 'job_single_page_id':
					$result = $this->find_page_id( $this->_job_single_page_id, JBP_JOB_VIRTUAL_KEY, JBP_JOB_SINGLE_FLAG );
					break;
				...
				case 'pro_archive_page_id':
					$result = $this->find_page_id( $this->_pro_archive_page_id, JBP_PRO_VIRTUAL_KEY, JBP_PRO_ARCHIVE_FLAG );
					break;
				case 'pro_taxonomy_page_id':
					$result = $this->find_page_id( $this->_pro_taxonomy_page_id, JBP_PRO_VIRTUAL_KEY, JBP_PRO_TAXONOMY_FLAG );
					break;
				case 'pro_contact_page_id':
					$result = $this->find_page_id( $this->_pro_contact_page_id, JBP_PRO_VIRTUAL_KEY, JBP_PRO_CONTACT_FLAG );
					break;
				case 'pro_search_page_id':
					$result = $this->find_page_id( $this->_pro_search_page_id, JBP_PRO_VIRTUAL_KEY, JBP_PRO_SEARCH_FLAG );
					break;
				case 'pro_single_page_id':
					$result = $this->find_page_id( $this->_pro_single_page_id, JBP_PRO_VIRTUAL_KEY, JBP_PRO_SINGLE_FLAG );
					break;
		...
		function __set( $name, $value ) {
			switch ( $name ) {
				case 'job_archive_page_id':
					$this->_job_archive_page_id = $value;
					break;
				case 'job_taxonomy_page_id':
					$this->_job_taxonomy_page_id = $value;
					break;
				case 'job_contact_page_id':
					$this->_job_contact_page_id = $value;
					break;
				case 'job_search_page_id':
					$this->_job_search_page_id = $value;
					break;
				case 'job_single_page_id':
					$this->_job_single_page_id = $value;
					break;
				case 'job_update_page_id':
				...
				case 'pro_archive_page_id':
					...
					break;
				case 'pro_taxonomy_page_id':
					$this->_pro_taxonomy_page_id = $value;
					break;
				case 'pro_contact_page_id':
					$this->_pro_contact_page_id = $value;
					break;
				case 'pro_search_page_id':
					$this->_pro_search_page_id = $value;
					break;
				case 'pro_single_page_id':
					$this->_pro_single_page_id = $value;
					break;
				case 'pro_update_page_id':
				...
				case 'demo_landing_page_id':
					$this->_demo_landing_page_id = $value;
					break;
			}
		}
		...
		function __isset( $name ) {
			...
			switch ( $name ) {
				case 'job_archive_page_id':
					$result = $this->_job_archive_page_id > 0;
					break;
				case 'job_taxonomy_page_id':
					$result = $this->_job_taxonomy_page_id > 0;
					break;
				case 'job_contact_page_id':
					$result = $this->_job_contact_page_id > 0;
					break;
				case 'job_search_page_id':
					$result = $this->_job_search_page_id > 0;
					break;
				case 'job_single_page_id':
					$result = $this->_job_single_page_id > 0;
					break;
				case 'job_update_page_id':
				...
				case 'pro_archive_page_id':
					...
					break;
				case 'pro_taxonomy_page_id':
					$result = $this->_pro_taxonomy_page_id > 0;
					break;
				case 'pro_contact_page_id':
					$result = $this->_pro_contact_page_id > 0;
					break;
				case 'pro_search_page_id':
					$result = $this->_pro_search_page_id > 0;
					break;
				case 'pro_single_page_id':
					$result = $this->_pro_single_page_id > 0;
					break;
				case 'pro_update_page_id':
				...
				case 'demo_landing_page_id':
					$result = $this->_demo_landing_page_id > 0;
					break;
			}
		...
		function register_scripts() {
			...
			if ( $custom_css_url ) {
				wp_register_style( 'jobs-plus-custom', $custom_css_url, array( 'jobs-plus' ), JOBS_PLUS_VERSION );
			}
			...
			if ( empty( $this->js_locale ) ) {
				$this->js_locale = 'en-US';
			}
		...
		function process_requests() {
			...
			if ( ! empty( $_REQUEST['jbp_error'] ) ) {
				sanitize_text_field( $this->error_message( $_REQUEST['jbp_error'] ) );
			}
			if ( ! empty( $_REQUEST['jbp_notice'] ) ) {
				sanitize_text_field( $this->notice_message( $_REQUEST['jbp_notice'] ) );
			}

			if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'verify' ) ) {
				return;
			}
		...
		function on_template_include( $template = '' ) {
			...
			if ( ! empty( $this->virtual ) ) {
				...
				if ( ! empty( $this->title ) ) {
					add_filter( 'the_title', array( &$this, $this->title ), 20, 2 );
				}
			}
		...
		function custom_titles( $title, $id = false ) {
			...
			if ( get_query_var( 'contact' ) ) {
				return $title . __( " &raquo; Contact", $this->text_domain );
			}
			...
			if ( is_search() && in_array( $post_type, array( 'jbp_job', 'jbp_pro' ) ) ) {
				switch ( $post_type ) {
					case 'jbp_pro':
						return esc_html( sprintf( '%s &raquo; Search &raquo; %s', $this->pro_labels->name, $_GET['s'] ) );
						break;
					case 'jbp_job':
						return esc_html( sprintf( '%s &raquo; Search &raquo; %s', $this->job_labels->name, $_GET['s'] ) );
						break;
					default:
						return $title;
				}
		...
		function on_parse_query( $query ) {
			...
			if ( is_singular() ) {
				$wp_post_statuses['virtual']->public = true;
			}
		...
		function is_certified( $user_id = 0 ) {
			if ( $this->get_setting( 'general->use_certification', false ) ) {
				return false;
			}
		...
		function on_ajax_set_jbp_certified() {
			if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'jbp-pro-update' ) ) {
				exit( 'No Nonce Sins' );
			}
		...
		function on_ajax_rate_pro() {
			...
			if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'rating' ) ) {
				exit;
			}
		...
		function on_ajax_jbp_pro() {
			...
			switch ( $name ) {
				...
				case 'post_content':
					$post['post_content'] = nl2br( wp_strip_all_tags( $value ) );
					break;
				...
				case '_ct_jbp_pro_Tagline':
					update_post_meta( $post_id, $name, json_encode( sanitize_text_field( $value ) ) );
					break;
				...
				case '_ct_jbp_pro_Skills':
				{
					...
					if ( empty( $value['skill'] ) || $value['remove'] == 'remove' ) {
						if ( isset( $group->$value['skill_id'] ) ) {
							unset( $group->$value['skill_id'] );
						}
						update_post_meta( $post_id, $name, json_encode( $group ) );
						// send back empty record.
						exit( '{"newValue": null}' );
					...
					if ( empty( $value['skill_id'] ) ) {
						$value['skill_id'] = uniqid();
					}
				...
				case '_ct_jbp_pro_Social':
				{
					...
					if ( ( $value['remove'] == 'remove' ) || empty( $value['social'] ) || empty( $value['url'] ) || empty( $value['social_id'] ) ) {
						if ( isset( $group->$value['social_id'] ) ) {
							unset( $group->$value['social_id'] );
						}
						update_post_meta( $post_id, $name, json_encode( $group ) );
						// send back empty record.
						exit( '{"newValue": null}' );
		...
		function on_ajax_jbp_pro_status() {
			...
			if ( ! $this->is_valid_pro_status( $params['post_status'] ) ) {
				exit;
			}
		...
		function job_contact_btn_sc( $atts, $content = null ) {
			...
			if ( $this->get_setting( 'job->disable_contact_email', false ) ) {
				return '';
			}
		...
		function expert_contact_btn_sc( $atts, $content = null ) {
			...
			if ( $this->get_setting( 'pro->disable_contact_email', false ) ) {
				return '';
			}
		...
		function job_list_btn_sc( $atts, $content = null ) {
			...
			if ( ! $url = $this->button_register_url() ) {
				$url = sprintf( '%s/author/%s/', untrailingslashit( get_post_type_archive_link( 'jbp_job' ) ), $user->user_login );
			}
		...
		function update_pro( &$params ) {
			...
			if ( ! current_user_can( EDIT_PRO, $params['post_id'] ) ) {
				return;
			}
		...
		function update_job( &$params ) {
			...
			//var_dump($params); exit;
			if ( ! current_user_can( EDIT_JOB, $params['data']['ID'] ) ) {
				return;
			}
		...
		function on_ajax_jbp_job() {
			...
			if ( ! wp_verify_nonce( $params['_wpnonce'], 'jbp_job' ) ) {
				...
				if ( $is_iframe ) {
					exit( sprintf( $response_iframe, '403', $statusText, $statusText . ': ' . print_r( $params, true ) ) );
				} else {
			...
			switch ( $name ) {
				...
				case 'post_content':
					$post['post_content'] = wp_strip_all_tags( $value );
					break;

				//Custom fields
				//json multi part fields

				case '_ct_jbp_job_Location'    :
					update_post_meta( $post_id, $name, sanitize_text_field( $value ) );
					break;
		...
		function on_ajax_jbp_job_status() {
			...
			if ( ! $this->is_valid_job_status( $params['post_status'] ) ) {
				exit;
			}
...
// Set flag on activation to trigger initial data
	...
	function jbp_flag_activation( $plugin = '' ) {
		...
		if ( $plugin == JBP_PLUGIN ) {
			update_site_option( 'jbp_activate', 2 );
		}
	}
