...
if ( ! class_exists( 'Jobs_Plus_Core_Data' ) ):
	...
	class Jobs_Plus_Core_Data {
		...
		function load_default_settings() {
			...
			if ( $settings === false ) {
				$settings = array( 'pro' => array(), 'job' => array() );
			}

			if ( ! isset( $settings['pro']['email_subject'] ) ) {
				$settings['pro']['email_subject'] = $GLOBALS['default_pro_subject'];
			}
			if ( ! isset( $settings['pro']['email_content'] ) ) {
				$settings['pro']['email_content'] = $GLOBALS['default_pro_content'];
			}

			if ( ! isset( $settings['job']['email_subject'] ) ) {
				$settings['job']['email_subject'] = $GLOBALS['default_pro_subject'];
			}
			if ( ! isset( $settings['job']['email_content'] ) ) {
				$settings['job']['email_content'] = $GLOBALS['default_job_content'];
			}

			if ( ! isset( $settings['pro']['moderation']['publish'] ) ) {
				$settings['pro']['moderation']['publish'] = 1;
			}
			if ( ! isset( $settings['job']['moderation']['publish'] ) ) {
				$settings['pro']['moderation']['publish'] = 1;
			}

			if ( ! isset( $settings['pro']['moderation']['publish'] ) ) {
				$settings['pro']['moderation']['draft'] = 1;
			}
			if ( ! isset( $settings['job']['moderation']['publish'] ) ) {
				$settings['pro']['moderation']['draft'] = 1;
			}
